...
1. Les vecteurs  
Un vecteur est une structure de donnée qui contient des éléments du même type (numéric, caractère, logique, etc.) Autrement dit , un vecteur ne peut pas contenir deux éléments de type différent. Pour définir un vecteur, on utilise la fonction c() qui signifie concaténation. Ainsi c(1,3,5) et c("Ali","Oumou","IB") sont des vecteurs valables tandis que c(1,"Oumar") n'est pas un vecteur valable.  
```{r vecteurs}
v1 <- c(1,3,5)
print(v1)
...
On peut créer un vecteur qui repète une certaine valeur à l'aide de la fonction **rep()**. Par exemple pour créer un vecteur qui contient 5 fois le mot "Oumar" on écrira:


```{r}
v4 <- rep("Oumar",times=5)
print(v4)
```
On peut également compter par exemple de 1 à 100 par pas 10 à l'aide de la fonction **seq()**
```{r}
v5 <- seq(1,100,10)
print(v5)
```
