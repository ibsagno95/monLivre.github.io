...
1. Numeric  
Par **Numeric**, il  comprend l'ensemble des nombres réels. Hormis les nombres complexes (nombre ayant une partie imaginaire de la fome z = a+ib , 2+3i par exemple), tous les nombres sont de classe **Numeric** et sont stockés en type **double** par défaut. C'est à dire qu'ils sont stockés avec une précision double (deux chiffres après la virgule). Par exemple , **5** est stocké par défaut comme **5.00**. 
```{r type_numeric}
num1 <- 5
num2 <- 3.14
class(num1)
...
Pour indiquer à R qu'on désigne un entier naturel , on l'accompagne de la lettre **L**. Par exemple on écrira **5L** pour stocker le chiffre **5**.
```{r type_entier}
num3 <- 5L
typeof(num3)
```
...
typeof(logic2)
```
Les valeurs logiques sont très utiles en R pour faire de la comparaison ou pour vérifier une assertion. Par exemple supposons qu'on veuille se rassurer que **10 < 5**.  La console nous retourne **FALSE**. On peut egalement se rassurer du type d'une variable avec le mot clé **is**.  
PAr exemple , pour savoir si notre variable **num1** est de type ** numeric** on va écrire *is.numeric(num1)*.
```{r comparaison}
10 < 5
...
Au delà des types de variables dans R , R dispose des structures de données qui facilitent le stockage et la manipulation des données. Ces structures de données sont composées individuellement des variables de même nature (vecteurs) ou de natures differentes (listes, Dataframe).  

1. Les vecteurs  
Les veceteurs permettent de stocker une liste ordonnée d'élements tous de même type. L'on pourrait stocker par exemple,des notes obtenus en Mathématiques par un groupe de 5 élèves. POur déclarer un vecteur , on utilise le mot clé **c()** . 
```{r notes}
notes <- c(10,5,2,5,12)
print(notes)
```

Dans l'exemple ci-dessus, **notes** est appelé un **objet**. On peut afficher la structure d'un objet sous R en utilisant le mot clé **str()**.  
```{r structure}
str(notes)
```

Cette sortie nous donnes les infos sur le type des élements stockés dans le vecteur (**num** pour numeric ), la longueur du vecteur (**[1:5]** donc 5 éléments) et le détail des valeurs. Une autre commande utile pour les vecteurs est le mot clé **length()** qui permet d'afficher la longueur d'un vecteur. 
```{r}
length(notes)
paste("la longueur du vecteur notes est ", length(notes), sep = "")
```
Nous pouvons également des élements de type **character** ou **logical** dans un vecteur . Imaginons que nous voulons maintenant stocker le prénom des 5 membres du groupes, leurs genres et "OUI/NON" s'ils sont végétariens. . Nous allons donc créer trois vecteurs à l'occurence.
```{r}
prenoms <- c("Ali","Oumar","Fanta","Abdoul","Mariame")
sexe <- c("M","M","F","M","F")
veg <- c(TRUE,FALSE,FALSE,FALSE,TRUE)
print(prenoms)
print(sexe)
print(veg)
```

 1.1 Manipulation des vecteurs
Les vecteurd sont mutables. Cela signifie qu'on peut les modifier soit en ajoutant des élements, ou en les supprimant.  

Pour afficher un  élément d'un vecteur , on précise son emplacement entre crochet. 
```{r affichage_des_elements}
#Afficher le deuxième élement du vecteur prenoms
prenoms[2]

#Afficher les trois premiers du vecteur prenoms
prenoms[1:3]
```

Pour ajouter un élement à un vecteur dejà existant, on précise l'indexe de l'emplacement entre crochet **[]**. Le vecteur prenoms initialement créé comporte 5 prénoms. Pour ajouter un nouveau prénom , on va donc l'ajouter à la 6ème position comme suit:
```{r}
prenoms[6] <- "Ibsa"
prenoms
```
Pour supprimer un élement d'un vecteur, on lui affecte la valeur NULL. 

Autres manières de generer des vecteurs 
vecteurs avec noms
seq
rep
